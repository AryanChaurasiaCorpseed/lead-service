...
//						leadDTO.setSource("Corpseed HO");
	...
	@Transactional
	public void industryV1(String industrydata) {
		try {
			List<Map<String, String>> projectSheetData = readProjectFile(industrydata);
			...
			for (Map<String, String> projectRow : projectSheetData) {
				...
				String category = projectRow.get("maincategory");
				str.add(category);
...
//	}
	...
	@Transactional
	public void industryV2(String industrydata) {
		try {
			List<Map<String, String>> projectSheetData = readProjectFile(industrydata);
			...
			for (Map<String, String> projectRow : projectSheetData) {
				String category = projectRow.get("maincategory");
				String subcategory = projectRow.get("subcategory");
				if(res.containsKey(category)) {
					Set<String> cat = res.get(category);
					cat.add(subcategory);
			...
			for(Entry<String, Set<String>> s:res.entrySet()) {
				Industry ind= industryRepo.findByName(s.getKey());
				System.out.println(ind);
				List<SubIndustry>subList = new ArrayList<>();
				for(String scat:s.getValue()) {
					SubIndustry si = new SubIndustry();
					si.setName(scat);
					subIndustryRepo.save(si);
					subList.add(si);
				}
				ind.setSubIndustry(subList);
	...
	@Transactional
	public void industryV3(String industrydata) {
		try {
			List<Map<String, String>> projectSheetData = readProjectFile(industrydata);
...
//				String category = projectRow.get("maincategory");
				String subcategory = projectRow.get("subcategory");
				String bussinessActivity = projectRow.get("bussinessactivity");

				if(res.containsKey(subcategory)) {
					Set<String> cat = res.get(subcategory);
					cat.add(bussinessActivity);
					res.put(subcategory, cat);
				}else {
					Set<String> cat =new HashSet<>();
					cat.add(bussinessActivity);
					res.put(subcategory, cat);
			...
			for(Entry<String, Set<String>> s:res.entrySet()) {
//				Industry ind= industryRepo.findByName(s.getKey());
				SubIndustry subInd= subIndustryRepo.findByName(s.getKey());
				System.out.println(subInd);
				List<SubSubIndustry>subsubList = new ArrayList<>();
				for(String scat:s.getValue()) {
					SubSubIndustry ssi = new SubSubIndustry();
					ssi.setName(scat);
					subSubIndustryRepo.save(ssi);
					subsubList.add(ssi);
				}
				subInd.setSubSubIndustry(subsubList);
				subIndustryRepo.save(subInd);
	...
	@Transactional
	public void industryV4(String industrydata) {
		try {
			List<Map<String, String>> projectSheetData = readProjectFile(industrydata);
...
//				String subcategory = projectRow.get("subcategory");
				String bussinessActivity = projectRow.get("bussinessactivity");
				String bussinessData = projectRow.get("productservice");

				if(res.containsKey(bussinessActivity)) {
					Set<String> cat = res.get(bussinessActivity);
					cat.add(bussinessData);
					res.put(bussinessActivity, cat);
				}else {
					Set<String> cat =new HashSet<>();
					cat.add(bussinessData);
					res.put(bussinessActivity, cat);
			...
			for(Entry<String, Set<String>> s:res.entrySet()) {
//				Industry ind= industryRepo.findByName(s.getKey());
  				SubSubIndustry subsubInd=subSubIndustryRepo.findByName(s.getKey());
				System.out.println(subsubInd);
				List<IndustryData>industryDataList = new ArrayList<>();
				for(String scat:s.getValue()) {
					IndustryData  industryData = new IndustryData();
					industryData.setName(scat);
					industryDataRepo.save(industryData);
					industryDataList.add(industryData);
				}
				subsubInd.setIndustryDataList(industryDataList);
				subSubIndustryRepo.save(subsubInd);
