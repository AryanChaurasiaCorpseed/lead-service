...
//===================================================================== optimized count  = = == =  
	...
	@Query(value = "SELECT count(*) FROM erp_leads el WHERE el.status_id in(:statusIds) and el.is_deleted =:b and create_date BETWEEN :d1 AND :d2 and el.assignee_id in(:userIds)", nativeQuery = true)
	long findCountByStatusIdInAndIsDeletedAndInBetweenDateAndAssigneeIdIn(List<Long> statusIds,boolean b,String d1,String d2,List<Long>userIds);

	@Query(value = "SELECT count(*) FROM erp_leads el WHERE el.status_id in(:statusIds) and el.is_deleted =:b and create_date BETWEEN :d1 AND :d2", nativeQuery = true)
	long findCountByStatusIdInAndIsDeletedAndInBetweenDate(List<Long> statusIds,boolean b,String d1,String d2); 

	
	@Query(value = "SELECT count(*) FROM erp_leads el WHERE el.is_deleted =:b and el.assignee_id in(:userId)and create_date BETWEEN :d1 AND :d2", nativeQuery = true)
	long findCountByAssigneeAndIsDeletedAndInBetweenDate(Long userId,boolean b,String d1,String d2);

	@Query(value = "SELECT count(*) FROM erp_leads el WHERE el.status_id in(:statusIds) and el.is_deleted =:b and el.assignee_id in(:userId)", nativeQuery = true)
	long findCountByStatusIdInAndAssigneeIdInAndIsDeleted(List<Long> statusIds,List<Long>userId,boolean b); 

	
	@Query(value = "SELECT count(*) FROM erp_leads el WHERE el.status_id in(:statusIds) and el.is_deleted =:b", nativeQuery = true)
	long findCountByStatusIdInAndIsDeleted(List<Long> statusIds,boolean b); 
	
	@Query(value = "SELECT count(*) FROM erp_leads el WHERE el.is_deleted =:b and el.assignee_id in(:userId)", nativeQuery = true)
	long findCountByAssigneeAndIsDeleted(Long userId,boolean b);
	...
	// =========================== optimize =========
	@Query(value = "SELECT l.id,l.lead_name,l.count,l.client_mob_no as client_contact_no,l.email as client_email,sd.status_name ,u.full_name as assignee_name,u.email as assignee_email,l.create_date,l.source,l.is_reopen_by_quality ,ua.email as reopen_by_quality , l.client_name , uc.full_name , up.full_name  FROM erp_leads l left join status_details sd on sd.id=l.status_id left join user u on u.id=l.assignee_id  left join user ua on ua.id=l.reopen_marked_by_id left join user up on up.id=l.updated_by_id  left join user uc on uc.id=l.created_by_id WHERE l.status_id in(:statusIds) and l.is_deleted =:b and create_date BETWEEN :d1 AND :d2 and l.assignee_id in(:userIds) ORDER BY id desc", nativeQuery = true)
	...
	@Query(value = "SELECT l.id,l.lead_name,l.count,l.client_mob_no as client_contact_no,l.email as client_email,sd.status_name ,u.full_name as assignee_name,u.email as assignee_email,l.create_date,l.source,l.is_reopen_by_quality ,ua.email as reopen_by_quality , l.client_name , uc.full_name , up.full_name  FROM erp_leads l left join status_details sd on sd.id=l.status_id left join user u on u.id=l.assignee_id  left join user ua on ua.id=l.reopen_marked_by_id left join user up on up.id=l.updated_by_id  left join user uc on uc.id=l.created_by_id WHERE l.is_deleted =:b and l.assignee_id in(:userId)and l.create_date BETWEEN :d1 AND :d2 ORDER BY id desc", nativeQuery = true)
	List<Object[]> findByAssigneeAndIsDeletedAndInBetweenDate(Long userId,boolean b,String d1,String d2);
	
	@Query(value = "SELECT l.id,l.lead_name,l.count,l.client_mob_no as client_contact_no,l.email as client_email,sd.status_name ,u.full_name as assignee_name,u.email as assignee_email,l.create_date,l.source,l.is_reopen_by_quality ,ua.email as reopen_by_quality , l.client_name , uc.full_name , up.full_name  FROM erp_leads l left join status_details sd on sd.id=l.status_id left join user u on u.id=l.assignee_id  left join user ua on ua.id=l.reopen_marked_by_id left join user up on up.id=l.updated_by_id  left join user uc on uc.id=l.created_by_id WHERE l.status_id in(:statusIds) and l.is_deleted =:b and l.assignee_id in(:userId) ORDER BY id desc", nativeQuery = true)
	List<Object[]> findByStatusIdInAndAssigneeIdInAndIsDeleted(List<Long> statusIds,List<Long>userId,boolean b); 
	
	@Query(value = "SELECT l.id,l.lead_name,l.count,l.client_mob_no as client_contact_no,l.email as client_email,sd.status_name ,u.full_name as assignee_name,u.email as assignee_email,l.create_date,l.source,l.is_reopen_by_quality ,ua.email as reopen_by_quality , l.client_name , uc.full_name , up.full_name FROM erp_leads l left join status_details sd on sd.id=l.status_id left join user u on u.id=l.assignee_id  left join user ua on ua.id=l.reopen_marked_by_id left join user up on up.id=l.updated_by_id  left join user uc on uc.id=l.created_by_id WHERE l.status_id in(:statusIds) and l.is_deleted =:b ORDER BY id desc", nativeQuery = true)
	List<Object[]> findByStatusIdInAndIsDeleted(List<Long> statusIds,boolean b);
	
	@Query(value = "SELECT l.id,l.lead_name,l.count,l.client_mob_no as client_contact_no,l.email as client_email,sd.status_name ,u.full_name as assignee_name,u.email as assignee_email,l.create_date,l.source,l.is_reopen_by_quality ,ua.email as reopen_by_quality , l.client_name , uc.full_name ,  up.full_name FROM erp_leads l left join status_details sd on sd.id=l.status_id left join user u on u.id=l.assignee_id  left join user ua on ua.id=l.reopen_marked_by_id left join user up on up.id=l.updated_by_id  left join user uc on uc.id=l.created_by_id WHERE l.is_deleted =:b and l.status_id in(:statusIds) and l.assignee_id in(:userId) ORDER BY id desc", nativeQuery = true)
	List<Object[]> findByAssigneeAndStatusIdInAndIsDeleted(Long userId,List<Long> statusIds,boolean b);
	
	@Query(value = "SELECT l.id,l.lead_name,l.count,l.client_mob_no as client_contact_no,l.email as client_email,sd.status_name ,u.full_name as assignee_name,u.email as assignee_email,l.create_date,l.source,l.is_reopen_by_quality ,ua.email as reopen_by_quality ,l.client_name , uc.full_name ,  up.full_name FROM erp_leads l left join status_details sd on sd.id=l.status_id left join user u on u.id=l.assignee_id  left join user ua on ua.id=l.reopen_marked_by_id left join user up on up.id=l.updated_by_id  left join user uc on uc.id=l.created_by_id WHERE l.status_id in(:statusIds) and l.is_deleted =:b and l.create_date BETWEEN :d1 AND :d2 ORDER BY id desc", nativeQuery = true)
	List<Object[]> findByStatusIdInAndIsDeletedAndInBetweenDate(List<Long> statusIds,boolean b,String d1,String d2); 
	
	@Query(value = "SELECT l.id,l.lead_name,l.count,l.client_mob_no as client_contact_no,l.email as client_email,sd.status_name ,u.full_name as assignee_name,u.email as assignee_email,l.create_date,l.source,l.is_reopen_by_quality ,ua.email as reopen_by_quality ,l.client_name , uc.full_name ,  up.full_name FROM erp_leads l left join status_details sd on sd.id=l.status_id left join user u on u.id=l.assignee_id  left join user ua on ua.id=l.reopen_marked_by_id left join user up on up.id=l.updated_by_id  left join user uc on uc.id=l.created_by_id WHERE l.is_deleted =:b and create_date BETWEEN :d1 AND :d2 and l.assignee_id in(:userIds) ORDER BY id desc", nativeQuery = true)
	List<Object[]> findByIsDeletedAndInBetweenDateAndAssigneeIdIn(boolean b,String d1,String d2,List<Long>userIds);

	@Query(value = "SELECT l.id,l.lead_name,l.count,l.client_mob_no as client_contact_no,l.email as client_email,sd.status_name ,u.full_name as assignee_name,u.email as assignee_email,l.create_date,l.source,l.is_reopen_by_quality ,ua.email as reopen_by_quality ,l.client_name , uc.full_name ,  up.full_name FROM erp_leads l left join status_details sd on sd.id=l.status_id left join user u on u.id=l.assignee_id  left join user ua on ua.id=l.reopen_marked_by_id left join user up on up.id=l.updated_by_id  left join user uc on uc.id=l.created_by_id WHERE l.is_deleted =:b and create_date BETWEEN :d1 AND :d2 ORDER BY id desc", nativeQuery = true)
	List<Object[]> findByIsDeletedAndInBetweenDate(boolean b,String d1,String d2);
     
	@Query(value = "SELECT l.id,l.lead_name,l.count,l.client_mob_no as client_contact_no,l.email as client_email,sd.status_name ,u.full_name as assignee_name,u.email as assignee_email,l.create_date,l.source,l.is_reopen_by_quality ,ua.email as reopen_by_quality ,l.client_name , uc.full_name ,  up.full_name FROM erp_leads l left join status_details sd on sd.id=l.status_id left join user u on u.id=l.assignee_id  left join user ua on ua.id=l.reopen_marked_by_id left join user up on up.id=l.updated_by_id  left join user uc on uc.id=l.created_by_id WHERE l.is_deleted =:b and l.assignee_id in(:userIds) ORDER BY id desc", nativeQuery = true)
	List<Object[]> findByIsDeleted(boolean b,List<Long>userIds);
	
	@Query(value = "SELECT l.id,l.lead_name,l.count,l.client_mob_no as client_contact_no,l.email as client_email,sd.status_name ,u.full_name as assignee_name,u.email as assignee_email,l.create_date,l.source,l.is_reopen_by_quality ,ua.email as reopen_by_quality ,l.client_name , uc.full_name ,  up.full_name FROM erp_leads l left join status_details sd on sd.id=l.status_id left join user u on u.id=l.assignee_id  left join user ua on ua.id=l.reopen_marked_by_id left join user up on up.id=l.updated_by_id  left join user uc on uc.id=l.created_by_id WHERE l.status_id =:statusId and l.is_deleted =:b ORDER BY id desc", nativeQuery = true)
	List<Object[]> findByStatusIdAndIsDeleted(Long statusId,boolean b);
	
	@Query(value = "SELECT l.id,l.lead_name,l.count,l.client_mob_no as client_contact_no,l.email as client_email,sd.status_name ,u.full_name as assignee_name,u.email as assignee_email,l.create_date,l.source,l.is_reopen_by_quality ,ua.email as reopen_by_quality ,l.client_name , uc.full_name ,  up.full_name FROM erp_leads l left join status_details sd on sd.id=l.status_id left join user u on u.id=l.assignee_id  left join user ua on ua.id=l.reopen_marked_by_id left join user up on up.id=l.updated_by_id  left join user uc on uc.id=l.created_by_id WHERE l.is_deleted =:b and l.assignee_id in(:userId) ORDER BY id desc", nativeQuery = true)
	List<Object[]> findByAssigneeAndIsDeleted(Long userId,boolean b);
