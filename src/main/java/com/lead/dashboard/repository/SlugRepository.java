...
public interface SlugRepository extends JpaRepository<Slug, Long> {
	...
	@Query(value = "SELECT us.urls_managment_id FROM finalerp.slug s left join finalerp.url_slug us on us.url_slug_id=s.id WHERE s.name LIKE %:searchTerm%", nativeQuery = true)
