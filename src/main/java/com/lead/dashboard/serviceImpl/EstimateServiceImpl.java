...
//			service.setUpdatedBy(user);
	...
	@Override
	public List<Map<String, Object>> searchEstimate(String search, Long userId) {
		User user = userRepo.findById(userId).get();
		List<Map<String, Object>>res=new ArrayList<>();
		List<String> role = user.getUserRole().stream().map(i->i.getName()).collect(Collectors.toList());
		if(role.contains("ADMIN")||role.contains("ACCOUNT")) {
			List<ServiceDetails> serviceDetails = serviceDetailsRepository.findByNameOrGST(search);
